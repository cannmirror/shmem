...
预配置的 9 技能工具链，覆盖 SHMEM 通信算子与通算融合算子从需求到交付的端到端开发，由显式的 Phase 0-7（含条件性 Phase 6.5）状态机驱动。

```
Phase 0 需求确认 → 1 设计 → 2 用例 → 3 代码 → 4 编译+正确性 → 5 代码走读 → 6 性能基线 → 6.5 性能优化（条件性） → 7 最终交付
```

本仓库提供

- `skill-format-guide.md` — 技能编写规范（frontmatter / 章节顺序 / 约束语言）
- `shmem-ops-dev/SKILL.md` — 编排器：Phase 0-7 全流程调度，子 skill 串行编排
- `shmem-ops-*/SKILL.md` — 8 个专职 skill，分别负责算子开发的一个阶段（设计 / 用例 / 代码 / 编译调试 / 正确性 / 走读 / 性能评估 / 性能优化）
...
> 现在需要你基于这个 moe dispatch 伪代码，在 custom-ops（自定义独立算子工程）目录下，基于 shmem 实现 moe dispatch 算子。目前不需要支持量化操作，数据类型 fp16，网络拓扑单机 8 卡，full-mesh 连接，p2p 带宽 28GB/s，conda 环境为 torch29py312。

3. 编排器自动按 Phase 0-7 推进，在每个 phase 调度对应 skill，输出可交付算子和全套报告。
...
Phase 5 代码走读  ───────────────────────── review-report.md
  ↓
Phase 6 性能基线评估  ────────────────────── performance_report.md
  ├─ 达标 ──→ Phase 7 最终交付
  └─ 未达标 ──→ Phase 6.5 性能优化（5 轮）──→ Phase 7 最终交付
```
...
| 2 | 用例生成 | `shmem-ops-testcase-gen` | `design.md` | case matrix + gen_data + checker + run.sh |
...
| 6 | 性能基线 | `shmem-ops-performance-eval` | 通过走读的算子 | `performance_report.md` |
| 6.5 | 性能优化 ⚠条件性 | `shmem-ops-performance-optim` | performance_report.md | 优化代码 + 轮次报告 |
...
| Skill | 类型 | Phase | 职责 |
|-------|------|-------|------|
| `shmem-ops-dev` | 编排器 | 0-7（含 6.5） | 推进 phase、强制门禁、调度子 skill、Phase 7 交付摘要。本身不直接做领域工作 |
...
| `shmem-ops-performance-eval` | 评测 | 6 | 性能采集、baseline 对比（HCCL/aclnn/拼接）、瓶颈分析、Device Frame Table |
| `shmem-ops-performance-optim` | 迭代优化 | 6.5（条件性） | 5 轮优化（Round 1 参数调优 + Round 2-5 机制优化），每轮：假设→改动→复测→决策 |
...
工作流：Phase 0 需求确认 → Phase 1 设计 → Phase 2 用例 → Phase 3 代码 →Phase 4 编译+正确性 → Phase 5 走读 → Phase 6 性能基线 → Phase 6.5 性能优化（条件性） → Phase 7 最终交付。

核心原则：八阶段+条件性子阶段串行、阶段门控、设计驱动编码、先 correctness 再 performance、性能优化上限 5 轮、不静默修改核心库。Phase 7 为编排器自处理的交付摘要，无专职 skill。
...
交付：case matrix（≥20 cases，XS/S/M/L 四档规模）、`gen_data.py`（golden 数据生成）、`check_result.py`（精度验证）、`run.sh`（批量运行脚本）。
...
支持两种算子类型：纯通信算子（`templates/communication/GUIDE.md`）和通算融合算子（`templates/fused-compute/GUIDE.md`）。模板以 fenced code block 承载在 GUIDE.md 中，Agent 提取后写入目标路径并替换占位符。
...
适用场景：采集算子性能数据，接入 baseline 对比，分析性能瓶颈，生成性能报告。

交付：`performance_report.md`，包含 Round 0 基线数据、Device Frame Table（pipeline timeline）、瓶颈分析、与 HCCL/aclnn/拼接 baseline 的对比。

### shmem-ops-performance-optim — 性能优化（Phase 6.5，条件性）

适用场景：算子正确性通过、基线已采集、Phase 6 未达标，需要系统化提升性能。

工作流：瓶颈定位 → 基线锁定 → 5 轮优化迭代（Round 1 参数调优 +Round 2-5 机制优化）→ 正确性复测 → 性能验证。

每轮记录：假设 → 代码/参数改动 → 正确性复测结果 → 性能采集 → 与 baseline 对比→ 决策（继续/回溯/停止）。全部 5 轮完成后，将最优参数合并到最终代码。

**注意**：此 Phase 仅在 Phase 6 性能基线未达标时进入；达标则直接跳至 Phase 7 最终交付。

### Phase 7 — 最终交付（编排器自处理）

适用场景：Phase 6 达标或 Phase 6.5 完成后，输出最终交付摘要。

交付：编排器汇总全部产出物，输出最终交付摘要，包含算子名称、目录结构校验、修改文件列表、编译命令、测试命令、正确性验证结果表、性能数据和 baseline 对比、优化轮次摘要（如有）、未完成项和风险。
...
每个算子在算子目录下生成。Phase 0-7（含条件性 Phase 6.5）完整走通后的文件结构：

```
<op_name>/
├── docs/
│   ├── design.md                  ← Phase 1：Canonical DSL + 能力映射 + 契约
│   ├── review-report.md           ← Phase 5：设计-代码一致性走读报告
...
├── shmem-ops-dev/
│   └── SKILL.md                           ← 编排器：Phase 0-7（含 6.5）全流程调度
├── shmem-ops-design/
│   ├── SKILL.md                           ← 算子设计
│   └── references/
...
│       ├── implementation-boundary.md     ←   Device/Host/Python 实现边界
...
│       ├── test-structure-template.md     ←   gen_data / check_result / run.sh 模板
...
│   └── templates/
│       ├── communication/GUIDE.md          ←   纯通信算子模板（fenced code block 承载）
│       └── fused-compute/GUIDE.md          ←   通算融合算子模板（fenced code block 承载）
├── shmem-ops-compile-debug/
│   ├── SKILL.md                           ← 编译与调试
│   └── references/
│       ├── build-test.md                  ←   构建模式 / CMake 模板 / 运行脚本
...
└── shmem-ops-performance-optim/
    ...
    └── references/
        ...
        └── compute-optimization.md        ←   计算层优化策略
```
...
- [skill-format-guide.md](skill-format-guide.md) — 技能编写规范（425 行）：frontmatter 定义、章节顺序、约束语言、skill 类型差异、写作清单
- [shmem-ops-dev/SKILL.md](shmem-ops-dev/SKILL.md) — 编排器完整文档（371 行）：核心原则、子 skill 清单、工作流总览、反模式清单
