...
> reduce 累加的正确模式和效率决策。适用于需要在多 PE 间进行 reduce 操作的通算融合和
> 集合通信算子。
...
5. `aclshmemi_copy_ub2gm(remote_ptr, buf, sizeof(T))` 触发 MTE3 写入
...
```
1. aclshmemx_barrier_all_vec()       // 确保所有 PE 的 workspace 已就绪
...
16. aclshmemx_barrier_all_vec()       // 通知所有 PE 本 PE 已完成读取
```
...
post aclshmemx_barrier_all_vec():
...
| ❌ 去掉 pre `aclshmemx_barrier_all_vec()` | 可能读到 remote PE 尚未写完的 workspace |
| ❌ 去掉 post `aclshmemx_barrier_all_vec()` | AIC 可能复用 stage，覆盖仍被 remote PE 读取的数据 |
...
| 配套机制 | 用途 |
| --- | --- |
| `aclshmemx_barrier_all_vec()` × 2 | pre: workspace 就绪；post: workspace 可复用 |
