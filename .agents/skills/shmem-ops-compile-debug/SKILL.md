...
| [references/log-debug.md](references/log-debug.md) | 初始化/运行时失败时 | SHMEM 日志环境变量、日志格式和阅读方法 |
| [references/dump-debug.md](references/dump-debug.md) | Device 数据/同步异常时 | AscendC DumpTensor/printf 调测 API 使用方法 |

必要时继续读取当前 SHMEM 仓库的 `README.md`、`scripts/build.sh`、`docs/compilation_build_guide.md`、`examples/CMakeLists.txt` 和相近 example 的 `CMakeLists.txt`/`run.sh`。真实代码优先于记忆。

## 输入契约

调用本 skill 前应尽量提供：

- `shmem_repo`：SHMEM 源码路径（默认 `shmem/`）
- `cann_env`：用户指定的 CANN `set_env.sh` 路径
- `python_cmd` 或 `python_env`：Python 可执行文件或环境激活命令
- `op_dir`：算子工程目录
- `build_mode`：`independent_project` 或 `in_tree_example`（缺失时自动判断）
- `design.md` 中的 compile/test/perf contract
- 目标 PE 数、device 列表、`ip_port`、engine、dtype/shape case

缺少环境信息时先向用户询问，不直接停止。只有用户明确无法提供时才标记环境阻塞。
...
| 模式 | 判断规则 |
| --- | --- |
| `in_tree_example` | `op_dir` 位于 `examples/<op_name>` |
| `independent_project` | `op_dir` 不在 shmem 仓库内 |
...
- ❌ `scripts/build.sh -examples` 失败后直接改全局 CMake，未定位目标 example
