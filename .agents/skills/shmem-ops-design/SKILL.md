...
当 API 名、能力边界、示例路径或约束不明确时，继续检查当前 SHMEM 仓库。优先看 `include/`、`src/`、`examples/`、`tests/`、`docs/` 下的真实代码和文档。
...
4. 按 [references/op-dsl.md](references/op-dsl.md) 填写 Canonical DSL。
...
1. **core_partition（最先设计）**：
   ...
   - `groups`：每组核的编号范围、职责、使用的数据接口。**职责描述必须精确到以下三要素，禁止笼统概括**：
     1. **AIV→数据区间映射**：每个 AIV 负责哪段数据，写出 `offset` 和 `len` 公式（及尾块处理方式，参考 core-allocation.md §5）
...
2. 查找 aclnn 融合算子：aclnnMatmulAllReduce、aclnnMatmulReduceScatter、aclnnAllGatherMatmul、**aclnnMatmulAllToAll**、aclnnAlltoAllV 等
3. 是否可用 HCCL + CATLASS/AscendC 串行拼接（如 AllToAll + MatMul）
4. 以上均无 → 填写 `"metric_only"` 并记录搜索过程

> **常见遗漏**：HCCL 库包含 AllToAll 算子（`HcclAlltoAll`），aclnn 库包含 MatmulAllToAll 等融合算子。设计时不要误认为这些算子没有 baseline。

### 算子实现边界

设计文档必须明确 Device kernel、`main.cpp`、Host helper、Python scripts 四层职责边界。详见 [references/implementation-boundary.md](references/implementation-boundary.md)。
...
- ❌ DSL `performance.baseline` 未逐步排查 HCCL 清单、aclnn 清单、拼接方案就直接写 `"metric_only"`
