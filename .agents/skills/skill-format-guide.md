...
- 全文恰好一个 H1
- 紧跟一行 Skill 类型声明（如有）：`**Skill类型**：流程导向型（八阶段工作流，子技能串行编排）`
...
需求环境确认 ──▶ 设计文档 ──▶ 用例生成 ──▶ 代码生成
—              design     testcase-gen   code-gen
...
​```bash
# Shell 命令
source set_env.sh && cmake .. && make -j
​```
...
| API 名称、代码片段、变量名 | 英文原文 | `aclshmem_put`、`design.md` |
