...
| 接口 | 说明 |
|------|------|
| `util_get_ffts_config` / `util_set_ffts_config` | 910B/C 算子内 FFTS 基址；950 可不调用（§3.3.7.4）。 |
...
| 平台 | 行为 |
|------|------|
| Ascend910B/C | init 时 `rtGetC2cCtrlAddr`；算子入口 `util_set_ffts_config(util_get_ffts_config())` |
...
| 910B/C barrier 异常 | 算子入口 `util_set_ffts_config(util_get_ffts_config())` |
