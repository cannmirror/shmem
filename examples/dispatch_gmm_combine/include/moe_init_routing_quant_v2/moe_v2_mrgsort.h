...
public:
    ...
    __aicore__ inline void SetOutput(AscendC::GlobalTensor<float> &gmOutput, AscendC::LocalTensor<float> &ubOutput);
...
__aicore__ inline void MoeV2Mrgsort::SetInput(AscendC::GlobalTensor<float> &gmInput, AscendC::LocalTensor<float> &ubInput)
{
...
__aicore__ inline void MoeV2Mrgsort::SetOutput(AscendC::GlobalTensor<float> &gmOutput, AscendC::LocalTensor<float> &ubOutput)
{
...
__aicore__ inline void MoeV2Mrgsort::MrgsortCompute()
{
    ...
    if (this->remainListNum == MERGE_LIST_TWO) {
        ...
        AscendC::MrgSort<float, true>(this->ubOutput, sortListTail, elementCountListTail, listSortedNums, validBitTail, 1);
    } else if (this->remainListNum == MERGE_LIST_THREE) {
        ...
        AscendC::DataCopy(this->ubOutput, this->tmpUbInputs[0],
...
__aicore__ inline void MoeV2Mrgsort::CopyOut()
{
    ...
    intriParams.blockLen = AscendC::GetSortLen<float>(curLoopSortedNum) * sizeof(float);
    SetWaitFlag<AscendC::HardEvent::V_MTE3>(AscendC::HardEvent::V_MTE3);
    AscendC::DataCopyPad(this->gmOutput[outOffset], this->ubOutput, intriParams);
    outOffset += AscendC::GetSortLen<float>(curLoopSortedNum);
}
