...
2. 在shmem/目录运行:
    ...
    UDMA 高阶 RMA 接口默认使用 `PIPE_MTE3` 下发 WQE，需要一段 UB scratch。默认配置为 `offset = 189 * 1024`、`ub_size = 128` 字节、`sync_id = 0`；如果样例或业务 kernel 需要复用这段 UB，可通过`aclshmemx_set_udma_config(offset, ub_size, sync_id)` 调整。`ub_size`必须不小于 128 字节，用于容纳当前 UDMA 数据搬移操作的一块完整WQE staging block。
